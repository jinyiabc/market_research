...
if __name__ == '__main__':
    ...
    """
    file_name = os.path.splitext(os.path.basename(__file__))[0]    # commecial_bank_straight_latest

    commercial_bank_chinabond = pd.read_csv(f'../resource/{file_name}.CSV',
    ...
    for i in range(length):
        ...
        for spot_curve in spot_curves:
            ...
            holding_1year_return.append(bond.bondYield(100, ql.Actual360(), ql.Compounded, ql.Annual, settlement_date + ql.Period(1, ql.Years)))
    ...
    commercial_bank_chinabond['tol_valuation_ratio1'] = (commercial_bank_chinabond['zero_curve_clean_price'] \
                                                       - commercial_bank_chinabond['estimated_clean_price_SHCH'] ).abs()\
    ...
    df2 = commercial_bank_chinabond.loc[:, ['Symbol','Latest Bond Rating','estimated_clean_price_SHCH','zero_curve_clean_price','log_linear_clean_price', \
                                      ...
                                      ]].to_csv(f'../resource/{file_name}_valuation-2.csv')
